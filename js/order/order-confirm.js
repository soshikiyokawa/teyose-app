...
function updateOrderPreviewBtnState(){
  ...
  // 納品場所は、これから届けてもらう発注のときだけ選ぶ（レシートは支払済みなので要らない）
  const placeWrap=document.getElementById('order-place-wrap');
  if(dueWrap) dueWrap.style.display = receipt ? 'none' : '';
  if(payWrap) payWrap.style.display = receipt ? '' : 'none';
  if(placeWrap) placeWrap.style.display = receipt ? 'none' : '';
...
function openOrderPreview(){
  ...
  // 納品場所（レシート取り込みの発注には無い）
  const delivery = receipt ? {place:'',address:''} : orderDeliveryOf(project);
  if(!receipt && delivery.place==='その他' && !delivery.address){
    alert('納品場所を入力してください。');return;
  }
  ...
  currentOrder={no,project,date,dueDate,dueAsap:!receipt&&orderDueAsap,costType,paymentMethod:payment,suppliers:sup.name,supplierObj:sup,items:orderItems,subtotal,tax,total:subtotal+tax,createdByName,
    deliveryPlace:delivery.place, deliveryAddress:delivery.address};
  ...
  document.getElementById('order-pdf-body').innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px">
      ...
      <div style="text-align:right;font-size:11px;color:#555;line-height:1.7"><div style="font-weight:800;font-size:13px;color:#2a1e0e">${COMPANY.name}</div><div>${COMPANY.zip} ${COMPANY.address}</div><div>TEL：${COMPANY.tel}</div><div style="color:#5c7a3e">${COMPANY.url}</div>${createdByName?`<div style="color:#2a1e0e">担当者：${esc(createdByName)}</div>`:''}</div>
    </div>
    <div style="background:#f7f3eb;border-radius:7px;padding:10px 12px;margin-bottom:16px;font-size:12px">
      ...
      ${receipt?'':`<div style="margin-top:4px"><span style="color:#888">納品場所：</span><strong>${esc(orderDeliveryLabel(currentOrder))}</strong></div>`}
    ...
    <div style="margin-top:14px;font-size:11px;color:#888;border-top:1px solid #e0d8c8;padding-top:10px">
      ${receipt
        ...
        : `納品場所：${esc(orderDeliveryLabel(currentOrder))}　／　ご納品の際は${
            delivery.place==='現場' ? '現場担当者' : '担当者'}へご連絡ください。`}
    </div>`;
...
async function confirmOrder(){
  ...
  if(placeSel){ placeSel.value='現場'; orderPlaceChanged(); }
  const placeOther=document.getElementById('order-place-other');
  if(placeOther) placeOther.value='';
