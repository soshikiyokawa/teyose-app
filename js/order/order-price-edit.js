...
async function refetchOrdersAndCost(){
  try{
    ...
    if(rows) orders = rows.map(r=>({id:r.id,no:r.no,project:r.project,date:r.date,dueDate:r.due_date,dueAsap:!!r.due_asap,
      ...
      status:r.status,receivedAt:r.received_at||'',priceEdits:r.price_edits||[],createdByName:r.created_by_name||'',deliveryPlace:r.delivery_place||'',deliveryAddress:r.delivery_address||''}));
