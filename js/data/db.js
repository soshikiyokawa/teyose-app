...
function rowToEstimate(r){
  ...
  return {id:r.id,title:r.title,no:r.no,date:r.date,expire:r.expire,status:r.status,type:r.type,
    ...
    completion:ci.completion||0,actualProfit:ci.actualProfit||0,ordersMemo:ci.ordersMemo||'',clientAddress:ci.clientAddress||'',
...
async function dbSaveEstimate(data){
  const row = {
    ...
    contract_info:{
      ...
      extras:data.extras||[],
      completion:data.completion||0,actualProfit:data.actualProfit||0,ordersMemo:data.ordersMemo||'',clientAddress:data.clientAddress||''
    },
