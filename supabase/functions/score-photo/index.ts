...
const CRITERIA = {
  ...
  points: [
    { title: "何の写真か一目で分かるか",   detail: "主役がはっきりしていること" },
    { title: "明るさ",                     detail: "暗すぎ・白飛び・逆光で見えなくなっていないこと" },
    { title: "構図",                       detail: "水平・垂直が取れていて、余計なものが写り込んでいないこと" },
    { title: "仕事のよさが伝わるか",       detail: "木の質感、納まり、手仕事のあと、現場の空気" },
    { title: "載せて困らないか",           detail: "人の顔がはっきり写っている、表札・車のナンバー・図面の個人情報、"
                                                 + "散らかった様子、安全上まずい状態が写っていないこと" },
  ],
  bands: [
  ...
  notes: [
    "見るところは上から重い順です。下にいくほど点数への効きは小さくなります。",
    "一言コメントには、よければ何がよいか、惜しければ何を直せばよいかを書かせています。",
...
function buildPrompt(): string {
  const pts = CRITERIA.points.map((p, i) => `${i + 1}. ${p.title}。${p.detail}`).join("\n");
  const bands = CRITERIA.bands
    .map((b) => `- ${b.from}〜${b.to} … ${b.label}`).join("\n");
